...
if __name__ == '__main__':
    ...
    key = chr(ord(' ') ^ most_common(data[::3]))
    key += chr(ord(' ') ^ most_common(data[1::3]))
    key += chr(ord(' ') ^ most_common(data[2::3]))

    print(sum(ord(c) for c in decrypt(data, key)))
